...
def check_password():
   ...
   if "password_correct" not in st.session_state:
       ...
       st.set_page_config(page_title="山根会計 専売システム",
...
class SupremeLegacyEngine:
   ...
   def get_legal_shares(has_spouse, heirs_info):
       ...
       has_child = any(h['type'] in ["子",

"孫（養子含む）"] for h in heirs_info)

       has_parent = any(h['type'] == "親" for

h in heirs_info) if not has_child else False

       has_sibling = any(h['type'] in ["兄弟姉妹（全血）",

"兄弟姉妹（半血）"] for h in heirs_info) if not

(has_child or has_parent) else False
...
shares.append(per_h)
           ...
           h_total_ratio = d("0.3333333333333333") if has_spouse else
...
d("1.0")
           ...
           h_total_ratio = d("0.25") if has_spouse else
...
d("1.0")
   ...
   def bracket_calc(a):
       ...
       elif a <= 30000000: return a * d("0.15") -

d("500000")

       elif a <= 50000000: return a * d("0.20") -

d("2000000")

       elif a <= 100000000: return a * d("0.30") -

d("7000000")

       elif a <= 200000000: return a * d("0.40") -

d("17000000")

       elif a <= 300000000: return a * d("0.45") -

d("27000000")

       elif a <= 600000000: return a * d("0.50") -
...
d("42000000")
   ...
   def get_tax(taxable_amt, has_spouse, heirs_info):
       ...
       s_ratio, h_shares = SupremeLegacyEngine.get_legal_shares(has_spouse,
...
SupremeLegacyEngine.bracket_calc(taxable_amt * share)
...
if check_password():
   ...
   st.set_page_config(page_title="SUMMIT v31.11 PRO",
...
-- TAB 1: 基本構成 --
   ...
   with tabs[0]:
       ...
       heir_count = c1.number_input("相続人の人数（配偶者除く）",

1, 10, 2, key="in_child")

       has_spouse = c2.checkbox("配偶者は健在",

value=True, key="in_spouse")

       heirs_info = []

       for i in range(heir_count):

           h_type = st.selectbox(f"相続人 {i+1} の続柄", ["子", "孫（養子含む）", "親", "兄弟姉妹（全血）", "兄弟姉妹（半血）"],
...
-- TAB 2: 一次財産詳細 --
   ...
   with tabs[1]:
       ...
       with col_a:
           ...
           v_home = st.number_input("特定居住用：評価額",

value=32781936, key="v_home")

           a_home = st.number_input("特定居住用：面積(㎡)", value=330, key="a_home")

           v_biz = st.number_input("特定事業用：評価額",

value=0, key="v_biz")

           a_biz = st.number_input("特定事業用：面積(㎡)", value=400, key="a_biz")

           v_rent = st.number_input("貸付事業用：評価額",

value=0, key="v_rent")

           a_rent = st.number_input("貸付事業用：面積(㎡)", value=200, key="a_rent")

           v_build = st.number_input("建物評価",

value=1700044, key="v_build")

           v_land_others = st.number_input("その他の土地",

value=0, key="v_land_others")

       with col_b:

           st.subheader("💵 金融・贈与財産")

           v_stock = st.number_input("有価証券",

value=45132788, key="v_stock")

           v_cash = st.number_input("現預金",

value=45573502, key="v_cash")

           v_ins = st.number_input("生命保険金",

value=3651514, key="v_ins")

           v_others = st.number_input("その他",

value=1662687, key="v_others")

           v_gift_3y = st.number_input("相続前贈与（3〜7年）", value=0,
...
key="v_gift_3y")
       ...
       with col_c:
           ...
           v_debt = st.number_input("債務",

value=322179, key="v_debt")

           v_funeral = st.number_input("葬式費用",

value=41401, key="v_funeral")
...
-- 共通計算ロジック（フルスタック） --
   ...
   land_eval = d(v_home) + d(v_biz) + d(v_rent) + d(v_land_others) -

total_red

   ins_ded = min(d(v_ins), d(5000000) * d(st_count))

   pure_as = land_eval + d(v_build) + d(v_stock) + d(v_cash) + d(v_ins) +
...
d(v_gift_tax_free)
   ...
   total_tax_1 = SupremeLegacyEngine.get_tax(taxable_1, has_spouse,
...
f"△{int(ins_ded):,}", f"500万円 ×

{st_count}名"],

           ["8", "債務および葬式費用",

f"△{int(v_debt + v_funeral):,}", ""],

           ["9", "生前贈与加算財産",

f"{int(v_gift_3y):,}", "3〜7年内贈与"],

           ["10", "相続時精算課税適用財産",

f"{int(v_gift_tax_free):,}", "持ち戻し加算"],
...
f"△{int(basic_1):,}", f"3000万+(600万×{st_count})"],
...
f"{int(total_tax_1):,}", "法定相続分による按分合算"],
...
-- TAB 4: 二次相続明細（省略なし復旧版） --
   ...
   with tabs[3]:
       ...
       tax_s_1 = d(0) if acq_s_1 <= limit_s else (total_tax_1 * ratio_s *
...
d("0.5"))
       ...
       s_spend_total = d(st.session_state.get("in_s_spend", 5000000))

* d(st.session_state.get("in_interval", 10))

       s_debt_2nd = d(st.session_state.get("in_debt2", 5000000))

       

       tax_p_2 = max(d(0), net_acq_s + s_own + s_gift_2 - s_spend_total -
...
s_debt_2nd)
       ...
       total_tax_2 = SupremeLegacyEngine.get_tax(taxable_2, False, child_only

if child_only else heirs_info)
...
f"△{int(s_spend_total):,}", f"期間：{st.session_state.get('in_interval',
...
f"△{int(basic_2):,}", f"相続人{c_count_2}名"],
...
-- TAB 5: 二次推移予測 --
   ...
   with tabs[4]:
       ...
       cp1.number_input("配偶者の固有財産",

value=50000000, key="in_s_own")

       cp1.number_input("生前贈与累計", value=0,

key="in_s_gift")

       cp1.slider("想定期間(年)",

0, 20, 10, key="in_interval")

       cp2.number_input("年間生活費",

value=5000000, key="in_s_spend")

       cp2.number_input("二次：債務・葬式費用",

value=5000000, key="in_debt2")
...
-- TAB 6: 精密分析結果（フルデータ復旧版） --
   ...
   with tabs[5]:
       ...
       def run_sim(s_ratio_val):
           ...
           t_s1 = d(0) if acq_s <= lim_s else (total_tax_1 * r *
...
d("0.5"))
           ...
           _, h_shares = SupremeLegacyEngine.get_legal_shares(has_spouse,
...
h['type'] not in ["子", "親"] else d("1.0")

                t_others1 += (total_tax_1 *
...
h_shares[i] * sur)
           ...
           net_s2_acq = acq_s - t_s1 + d(s_own) + d(s_gift_2) - s_spend_total -

d(s_debt_2nd)

           t2 = SupremeLegacyEngine.get_tax(max(d(0), net_s2_acq - basic_2), False,

child_only if child_only else heirs_info)
       ...
       fig.add_trace(go.Bar(x=df_sim['配分(%)'],

y=df_sim['一次相続税額'], name='一次相続税',

marker_color='#1f2c4d'))

       fig.add_trace(go.Bar(x=df_sim['配分(%)'],

y=df_sim['二次相続税額'], name='二次相続税',

marker_color='#c5a059'))

       fig.add_trace(go.Scatter(x=df_sim['配分(%)'],

y=df_sim['合計納税額'], name='合計',

line=dict(color='#a61d24', width=4)))

       fig.update_layout(barmode='stack', title="配偶者取得割合別の税額推移", xaxis_title="配偶者の取得割合(%)",
...
yaxis_title="税額(円)")
       ...
       <div style="background-color: #001f3f; color: #d4af37; padding:

25px; border-radius: 10px; border: 2px solid #d4af37;">
